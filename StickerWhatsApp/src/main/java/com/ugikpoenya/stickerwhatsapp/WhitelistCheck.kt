...
object WhitelistCheck {
    ...
    private fun isWhitelistedFromProvider(context: Context, identifier: String, whatsappPackageName: String): Boolean {
        ...
        if (isPackageInstalled(whatsappPackageName, packageManager)) {
            ...
            val providerInfo = packageManager.resolveContentProvider(whatsappProviderAuthority, PackageManager.GET_META_DATA) ?: return false
            // provider is not there. The WhatsApp app may be an old version.
            val queryUri = Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(whatsappProviderAuthority).appendPath(QUERY_PATH).appendQueryParameter(AUTHORITY_QUERY_PARAM, context.packageName + ".stickercontentprovider").appendQueryParameter(IDENTIFIER_QUERY_PARAM, identifier).build()
